<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 097 プロジェクトバナー" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 097｜レトロ説明カード
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 097 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

写真の複雑な情報をすべて描く必要はありません。Panel 097 はそれを、古い説明書、広告カード、包装の中紙のように機能する図解へ圧縮します。

**Panel 097** は economy of line、平面的な模式遠近、少数の幾何形で主体の同一性を保ちます。深い黒が輪郭と情報階層を担い、第二の特色は写真から抽出され、節度ある two-colour spot printing をつくります。手描きの商業文字、指示線、少し不器用な機能的ユーモアによって、「説明すること」自体が画面になります。

### 向いている用途

- **一目で分かる図解が必要：** 輪郭、箱、穴、パネル、方向関係で主体と物語を説明します。
- **古い印刷の力強さが好き：** 深い黒と写真由来の第二特色を、暖かな紙と軽い版ずれ粒子に組み合わせます。
- **気取らないユーモアが欲しい：** レトロな商業手描き文字と補助記号を物語のためだけに使います。
- **柔軟に納品したい：** 上下・左右・デザインのみ・複数比率・四端末壁紙・フォルダ一括処理に対応します。

### すぐ試す

> この画像を XXD Panel 097 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** ミッドセンチュリー商業グラフィック · 模式線画 · 線の経済性 · 二色特色 · 手描き文字 · 機能的ユーモア
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 古い説明カードのように明快で節度があり、少しユーモラスな二色機能図解 |
| **識別しやすい特徴** | 機能的輪郭、模式遠近、深い黒と写真由来の特色、商業手描き文字、軽いユーモア |
| **入力の尊重** | 重要な同一性と場面関係を保ち、複雑な背景を削り、位置・密度・リズムを再構成 |
...
```text
主体・輪郭・構造・姿勢・物語関係を分析 → 少数の幾何形と機能線へ圧縮 → 深い黒と写真由来の第二特色で二色印刷を構築 → 商業手描き文字と指示線で情報とユーモアを補う
```

## 完成作の特徴

- 深い黒、暖かな紙白、写真由来の第二特色を使い、二色の模式線描を常に中心にします。
- 線は economy of line に従い、第二特色を機械的に赤へ固定しません。
- 主体を説明するために必要な卓面、地面、境界、穴、パネル、方向だけを残します。
- 商業手描き文字と引出線は少なく有用にし、装飾ではなく図解へ統合します。
- 写実、太い漫画線、滑らかなベクター、カートゥーン、過剰な細部、コンピュータフィルター、テンプレートポスターを避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-097-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 097 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-097 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-097 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-097 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-097 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 097 を適用し、共通設定は一度だけ解決
/xxd-panel-097 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-097`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-097-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-097-prompt.zh-CN.md)
...
| **兵士級** | `xxd-panel-NNN`（現在 001–097） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
