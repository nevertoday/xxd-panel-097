<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 097 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 097이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사진의 복잡한 정보를 전부 묘사할 필요는 없습니다. Panel 097은 이를 오래된 설명서, 광고 카드, 포장 삽지처럼 기능하는 도해로 압축합니다.

**Panel 097**은 economy of line, 평면적인 도식 원근, 소수의 기하 형태로 주체의 정체성을 지킵니다. 짙은 검정이 윤곽과 정보 층위를 맡고, 두 번째 별색은 사진에서 추출되어 절제된 two-colour spot printing을 만듭니다. 손그림 상업 글자, 지시선, 약간 서툰 기능적 유머가 ‘설명하는 일’ 자체를 화면으로 바꿉니다.

### 이런 경우에 적합합니다

- **한눈에 이해되는 도해가 필요할 때:** 윤곽, 상자, 구멍, 패널, 방향 관계로 주체와 이야기를 설명합니다.
- **옛 인쇄의 힘을 좋아할 때:** 짙은 검정과 사진에서 뽑은 두 번째 별색을 따뜻한 종이와 미세한 판 어긋남에 결합합니다.
- **과장되지 않은 유머가 필요할 때:** 복고 상업 손글씨와 보조 기호를 오직 서사를 위해 사용합니다.
- **유연한 납품이 필요할 때:** 상하·좌우·디자인 전용·여러 비율·4종 기기 벽지·폴더 일괄 처리에 대응합니다.

### 빠른 시작

> 이 이미지를 XXD Panel 097로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 미드센추리 상업 그래픽 · 도식 선화 · 선의 경제성 · 2색 별색 · 손글씨 · 기능적 유머
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 오래된 설명 카드처럼 명확하고 절제되며 약간 유머러스한 2색 기능 도해 |
| **대표 특징** | 기능 윤곽, 도식 원근, 짙은 검정과 사진 유래 별색, 상업 손글씨, 가벼운 유머 |
| **입력 존중 방식** | 핵심 정체성과 장면 관계를 지키고 복잡한 배경을 덜어 위치·밀도·리듬을 재구성 |
...
```text
주체·윤곽·구조·자세·서사 관계 분석 → 소수의 기하와 기능선으로 압축 → 짙은 검정과 사진에서 뽑은 두 번째 별색으로 2색 인쇄 구성 → 상업 손글씨와 지시선으로 정보와 유머 보완
```

## 완성작의 특징

- 짙은 검정, 따뜻한 종이 흰색, 사진에서 뽑은 두 번째 별색을 사용하며 2색 도식 선화를 중심에 둡니다.
- 선은 economy of line을 따르고 두 번째 별색을 기계적으로 빨강에 고정하지 않습니다.
- 주체를 설명하는 데 필요한 상판, 바닥, 경계, 구멍, 패널, 방향 단서만 남깁니다.
- 상업 손글씨와 지시선은 적고 유용하게 두며 장식이 아니라 도해에 통합합니다.
- 사실주의, 굵은 만화선, 매끈한 벡터, 카툰, 과도한 세부, 컴퓨터 필터, 템플릿 포스터를 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-097-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 097의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-097 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-097 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-097 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-097 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-097 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-097 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 097 적용, 공통 설정은 한 번만 확정
/xxd-panel-097 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-097`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-097-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-097-prompt.zh-CN.md)
...
| **[xxd-panel-097](https://github.com/nevertoday/xxd-panel-097)** | Mid-century vernacular commercial graphic · schematic line drawing · economy of line · extracted second spot colour · hand lettering · functional humour |
...
| **병사급** | `xxd-panel-NNN` (현재 001–097) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
