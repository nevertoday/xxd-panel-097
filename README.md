<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 097 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 097 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、结构、姿态与叙事关系 → 压缩为少量几何和功能线 → 以深黑与照片提炼的第二专色建立双色印刷 → 用手绘商业字和指示线补充信息与幽默
```

## 成品中最容易识别的风格特点

- 画面以深黑、暖纸白和一枚从照片提炼的第二专色为主，双色示意线描始终是核心。
- 线条遵循 economy of line，第二专色从照片提炼；不机械固定为红色。
- 复杂背景只保留必要的桌面、地面、边界或空间线索，让留白与排线节奏共同呼吸。
- 文字小、轻、克制，沿留白、边界或线性结构自然分布，不形成标题栏或菜单信息。
- 拒绝写实插画、粗黑漫画线、整洁矢量线、卡通、过度细节、电脑滤镜和模板化海报。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-097-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–097） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
